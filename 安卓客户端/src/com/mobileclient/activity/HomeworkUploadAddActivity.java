...
public class HomeworkUploadAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.homeworkupload_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加上传的作业");
		...
		iv_homeworkFile.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(HomeworkUploadAddActivity.this,photoListActivity.class);
		...
		btnAdd.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					if(homeworkUpload.getHomeworkFile() != null) {
						//如果图片地址不为空，说明用户选择了图片，这时需要连接服务器上传图片
						HomeworkUploadAddActivity.this.setTitle("正在上传图片，稍等...");
						String homeworkFile = HttpUtil.uploadFile(homeworkUpload.getHomeworkFile());
						HomeworkUploadAddActivity.this.setTitle("图片上传完毕！");
						homeworkUpload.setHomeworkFile(homeworkFile);
					} else {
						homeworkUpload.setHomeworkFile("upload/noimage.jpg");
					}
					/*验证获取提交时间*/ 
					if(ET_uploadTime.getText().toString().equals("")) {
						Toast.makeText(HomeworkUploadAddActivity.this, "提交时间输入不能为空!", Toast.LENGTH_LONG).show();
						ET_uploadTime.setFocusable(true);
						ET_uploadTime.requestFocus();
						return;	
					}
					homeworkUpload.setUploadTime(ET_uploadTime.getText().toString());
					if(homeworkUpload.getResultFile() != null) {
						//如果图片地址不为空，说明用户选择了图片，这时需要连接服务器上传图片
						HomeworkUploadAddActivity.this.setTitle("正在上传图片，稍等...");
						...
						homeworkUpload.setResultFile("upload/noimage.jpg");
					}
					/*验证获取批改时间*/ 
					if(ET_pigaiTime.getText().toString().equals("")) {
						Toast.makeText(HomeworkUploadAddActivity.this, "批改时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_pigaiFlag.getText().toString().equals("")) {
						Toast.makeText(HomeworkUploadAddActivity.this, "是否批改输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_pingyu.getText().toString().equals("")) {
						Toast.makeText(HomeworkUploadAddActivity.this, "评语输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传上传的作业信息*/
					HomeworkUploadAddActivity.this.setTitle("正在上传上传的作业信息，稍等...");
					String result = homeworkUploadService.AddHomeworkUpload(homeworkUpload);
