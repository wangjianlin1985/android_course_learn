...
public class VideoEditActivity extends Activity {
			...
			public void onClick(View v) {
				try {
					...
					if(ET_title.getText().toString().equals("")) {
						Toast.makeText(VideoEditActivity.this, "视频资料标题输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_path.getText().toString().equals("")) {
						Toast.makeText(VideoEditActivity.this, "文件路径输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_addTime.getText().toString().equals("")) {
						Toast.makeText(VideoEditActivity.this, "添加时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传视频信息信息*/
					VideoEditActivity.this.setTitle("正在更新视频信息信息，稍等...");
	...
	private void initViewData() {
		...
		for (int i = 0; i < chapterList.size(); i++) {
			if (video.getChapterId() == chapterList.get(i).getId()) {
				this.spinner_chapterId.setSelection(i);
				break;
			}
		}
		this.ET_path.setText(video.getPath());
		this.ET_addTime.setText(video.getAddTime());
	}
	@Override
